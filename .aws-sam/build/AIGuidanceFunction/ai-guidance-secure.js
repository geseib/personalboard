 ...
 * SECURE VERSION: Validates CloudFront secret header
 */
exports.handler = async (event) => {
  // Get allowed origin from environment variable
  const allowedOrigin = process.env.ALLOWED_ORIGIN || '*';
  const secretHeaderName = process.env.SECRET_HEADER_NAME || 'X-CloudFront-Secret';
  const secretHeaderValue = process.env.SECRET_HEADER_VALUE;
  ...
  // SECURITY: Validate secret header if configured
  if (secretHeaderValue) {
    const providedSecret = event.headers?.[secretHeaderName] || event.headers?.[secretHeaderName.toLowerCase()];
    
    if (providedSecret !== secretHeaderValue) {
      console.warn('Invalid or missing secret header');
      return {
        statusCode: 403,
        headers,
        body: JSON.stringify({ error: 'Forbidden', message: 'Invalid request origin' })
      };
    }
  }
  ...
  if (allowedOrigin !== '*' && origin !== allowedOrigin) {
    console.warn(`Invalid origin: ${origin}, expected: ${allowedOrigin}`);
    return {
      statusCode: 403,
      headers,
  ...
  try {
    ...
    // Log request type for monitoring (no sensitive data)
    console.log(`Processing guidance request: ${type}`);
